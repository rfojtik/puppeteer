...
class NetworkManager extends EventEmitter {
   ...
   */
  _handleRequestRedirect(request, responsePayload, monotonicTimestamp) {
    const response = new Response(this._client, request, responsePayload, monotonicTimestamp);
  ...
  _onResponseReceived(event) {
    ...
    if (!request)
      return;
    const response = new Response(this._client, request, event.response, event.timestamp);
    request._response = response;
    this.emit(Events.NetworkManager.Response, response);
  }
...
class Request {
  ...
  constructor(client, frame, interceptionId, allowInterception, event, redirectChain) {
    ...
    this._monotonicTimeStart = event.timestamp;
  }
...
class Response {
   ...
   * @param {?number} monotonicTimestamp
   */
  constructor(client, request, responsePayload, monotonicTimestamp) {
    this._client = client;
